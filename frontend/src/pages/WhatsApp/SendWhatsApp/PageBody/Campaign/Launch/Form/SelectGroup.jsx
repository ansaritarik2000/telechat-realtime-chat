...
import { getGroupContactsByGroupIdsService, getGroupsService } from "../../../../../../../services/phonebook/phonebookService";
...
const SelectGroup = ({ type = "" }) => {
  ...
  const { setPhoneNumbers: setWhatsAppPhoneNumbers,setContactUploadedFrom, groups,
    setGroups } = useSendWhatsappStore();
  ...
  const groupContacts = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await getGroupContactsByGroupIdsService(token, params);
...
//   useEffect(() => {
//     return () => {
//       // This will run when component unmounts (when changing tabs)
//       setSelectedGroups([]);
//     };
//   }, [setSelectedGroups]);
    ...
    <>
      <Select
        ...
        onSelectionChange={(keys) => {
          setContactUploadedFrom("phonebook");
