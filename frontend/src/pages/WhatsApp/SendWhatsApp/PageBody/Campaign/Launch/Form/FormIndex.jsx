...
export default function CampaignForm() {
  ...
  useEffect(() => {
    if ([...selectedTemplate].length > 0) {
      ...
      const getTemplateWithContent = async () => {
        const templateWithContent = await getWhatsappTemplateWithDetails({
          token,
        ...
        if (templateWithContent.status === "SUCCESS") {
          ...
          addToast({
            ...
            description: templateWithContent?.message || "Something went wrong",
  ...
  const onOpenAudienceModal = () => {
    ...
    setNumdisable(singleTemplateData[0]?.[contentType]?.variables?.length > 0);
    onOpen(); // open modal
  };
        ...
        if (!selectedDate) {
          ...
          if (!proccessResponse?.success) {
            throw {
              type: "meta",
              message:
                proccessResponse.error?.message || "Failed to send campaign",
        ...
        <CardBody className="p-6">
          <div className="w-full flex flex-col gap-8">
            ...
            <div className="flex justify-between gap-6 w-full">
              ...
              >
                {wabaids.map((waba) => (
                  <SelectItem key={waba.key}>{waba.label}</SelectItem>
                ))}
              </Select>
              {/* Template Type */}
              <Select
              ...
              >
                ...
                >
                  {templates.map((temp) => (
                    <SelectItem key={temp.key}>{temp.label}</SelectItem>
                  ))}
                </SelectSection>
                <SelectSection
                  classNames={{
                    ...
                    <>
                      <ModalBody className="flex flex-col h-full">
                        ...
                        >
                          ...
                          >
                            {contactUploadedFrom &&
                              contactUploadedFrom !== "csv" && (
                                <SelectAudienceAlert />
                              )}

                            {(!contactUploadedFrom ||
                              contactUploadedFrom === "csv") && <CsvUpload />}
                          </Tab>
                          {/* Copy paste Numbers */}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-end -mb-12 text-xs text-default-400 ">
        ...
        <span>Targeted Audience: {phoneNumbers && phoneNumbers.length}</span>
      </div>
    </div>
  );
};
