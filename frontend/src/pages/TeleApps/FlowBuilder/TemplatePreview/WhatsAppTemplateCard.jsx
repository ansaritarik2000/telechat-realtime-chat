...
const WhatsAppTemplateCard = ({
  ...
  return (
    <div className="relative border-gray-300 border rounded-tr-xl rounded-br-xl rounded-bl-xl rounded-tl-none p-3 flex flex-col gap-2 bg-background">
      <div className="absolute top-0 -left-4 w-0 h-0 border-b-[20px] border-gray-300 border border-l-0 border-r-[20px] border-b-transparent border-r-background"></div>
      ...
      <div className="p-2 pt-1 text-left">
        <h3 className="text-sm font-semibold mb-1">{title || card_heading}</h3>
        ...
        <div className="space-y-2">
          {(buttons || action_buttons)?.map((button, index) => (
            ...
            >
              {button.label || button.suggestion_text}
            </button>
          ))}
        </div>
      </div>
