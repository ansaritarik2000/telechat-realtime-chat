...
export default function IncomingWAConfig({ routeBack, nodeId }) {
    // Create separate state for each select
    const [selectedNumber, setSelectedNumber] = React.useState(new Set([]));
    ...
    return (
        <div className="w-fit">
            ...
            <div className="flex flex-col gap-3  mt-3">
                ...
                >
                    {wanumbers.map((wanum) => (
                        <SelectItem key={wanum.key}>{wanum.number}</SelectItem>
                    ))}
                </Select>
