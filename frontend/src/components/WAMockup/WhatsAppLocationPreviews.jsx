...
const WhatsAppLocationPreviews = () => {
  const { location } = useWhatsappTemplateStore();
  return (
    <>
      ...
      <div>
        {" "}
        <p className="text-semibold text-sm break-all">{location.textMessage }</p>
        <p className="text-default-400  text-xs break-all">{location.FooterText}</p>
      </div>
      <div>
        {location?.buttons &&
