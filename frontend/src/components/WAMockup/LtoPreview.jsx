...
const LtoPreview = () => {
  const { LtoContent , mediaType} = useWhatsappTemplateStore();
  return (
    <>
      ...
      <div>
        {" "}
        <p className="text-semibold text-sm break-all">{LtoContent?.textMessage}</p>
        {/* <p className="text-default-400  text-xs">{LtoContent?.FooterText}</p> */}
      </div>
      <div>
        {LtoContent?.buttons &&
          LtoContent?.buttons?.map((btn, index) => {
            let icon;
