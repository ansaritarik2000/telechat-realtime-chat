...
const RowSteps = React.forwardRef(
  (
    ...
    return (
      <nav
        aria-label="Progress"
        className="w-full justify-center overflow-x-auto py-1"
        // className="-my-4 max-w-fit overflow-x-scroll py-4"
      >
        <ol
          className={cn("flex flex-row flex-nowrap gap-x-1", colors, className)}
        >
          {steps?.map((step, stepIdx) => {
            ...
            return (
              <li
                key={stepIdx}
                className="relative flex w-full items-center pr-2"
              >
                <button
                  key={stepIdx}
                  ref={ref}
                ...
                >
                  ...
                  {stepIdx < steps.length && !hideProgressBars && (
                    <div
                      aria-hidden="true"
